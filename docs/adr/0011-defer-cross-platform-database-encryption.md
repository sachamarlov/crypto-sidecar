...
_every_ platform, without making the install path painful.
...
- **C. Layered strategy (chosen)**:
  ...
  2. On any platform where SQLCipher is **not** present, fall back to
     **column-level AES-GCM encryption** of the sensitive columns
     ...
     using a key derived from the _vault administrator_ password — same
...
because it covers _every_ SQLite page (including B-tree indices and
